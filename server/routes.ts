    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Интеллектуальные ответы для имитации работы ChatGPT
          if (content.toLowerCase().includes('привет') || content.toLowerCase().includes('здравствуй')) {
            aiResponse = "Здравствуйте! Чем я могу вам помочь сегодня?";
          } else if (content.toLowerCase().includes('погода')) {
            aiResponse = "К сожалению, у меня нет доступа к данным о текущей погоде. Но я могу помочь вам с другими вопросами!";
            ...
            aiResponse = "Я ассистент на базе искусственного интеллекта, созданный для помощи и ответов на вопросы. Чем могу быть полезен?";
          } else if (content.toLowerCase().includes('есть')) {
            aiResponse = "Да, я здесь и готов помочь вам! Что вас интересует?";
          } else if (content.toLowerCase().includes('спасибо') || content.toLowerCase().includes('благодар')) {
            aiResponse = "Пожалуйста! Рад был помочь. Если у вас возникнут еще вопросы, не стесняйтесь спрашивать.";
          } else if (content.toLowerCase().includes('батуми') || content.toLowerCase().includes('недвижимость')) {
            aiResponse = "Батуми - прекрасный город на побережье Черного моря в Грузии. Недвижимость там пользуется большим спросом среди инвесторов. Средняя стоимость квартиры составляет примерно 1000-1500$ за квадратный метр в зависимости от расположения и качества. Могу рассказать подробнее, если интересно.";
          } else if (content.toLowerCase().includes('помоги') || content.toLowerCase().includes('помощь')) {
            aiResponse = "Конечно, я с удовольствием помогу! Чтобы я мог предоставить наиболее полезную информацию, уточните, пожалуйста, ваш вопрос или задачу.";
          } else if (content.toLowerCase().includes('расскажи') || content.toLowerCase().includes('объясни')) {
            aiResponse = `Конечно, я с удовольствием расскажу про "${content.replace(/расскажи|объясни/gi, '').trim()}".\n\nЭто очень интересная тема, которая включает в себя множество аспектов. Важно понимать основные принципы и концепции, чтобы получить полное представление. Давайте рассмотрим ключевые моменты:\n\n1. История вопроса\n2. Основные компоненты\n3. Практическое применение\n\nХотите, чтобы я углубился в какой-то конкретный аспект?`;
          } else {
            // Более умный запасной ответ для общих вопросов
            aiResponse = `Я понял ваш вопрос про "${content}". Это интересная тема, которую можно рассмотреть с разных сторон. Чтобы предоставить вам наиболее полезную информацию, я бы хотел уточнить, какой именно аспект вас интересует больше всего?`;
          }
        }
      } catch (webhookError) {
